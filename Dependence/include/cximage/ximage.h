...
public:
	...
	static bool GetHeaderInfo(const TCHAR* filename, 
		DWORD imagetype, long &width,long &height);

	static bool GetHeaderInfo(CxFile* hFile, 
		DWORD imagetype, long &width,long &height);

	//added by leven
	// 注意CXiamge的原点坐标在左下角,此时注意x和y的取值.
	bool Encode2RGB(CxFile *hFile);
	bool Encode2RGB(BYTE * &buffer, long &size);
	bool EncodeToA8R8G8B8(CxFile *hFile);
	bool EncodeToA8R8G8B8(BYTE * &buffer, long &size);

	bool Encode2BGRA(BYTE * &buffer, long &size);

	void MixFrom2(CxImage & imagesrc2, long lXOffset, long lYOffset);
	void MixFrom3(CxImage & imagesrc2, long lXOffset, long lYOffset);
	//------------------------------------------
	// leven add end
	//------------------------------------------
};
